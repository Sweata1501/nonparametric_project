                                  ...
                                  repl = 1000, p = 0.05)
{
  ...
  colnames(reject_prob) <- nvec; rownames(reject_prob) <- mvec; 
  for(i in nvec){
    for(j in mvec){
  ...
  close(pb) #pb
  write.xlsx(reject_prob, sprintf("data/%s -- theta=%.3f -- %s -- n-%s -- m-%s.xlsx", 
                                 as.character(substitute(stat_rejection)),
                                 theta, as.character(substitute(rdist)), 
                                 paste(nvec[1],nvec[length(nvec)], collapse = ", "), paste(mvec[1],mvec[length(mvec)], collapse = ", ")))
  reject_prob
}
                             ...
                             p = 0.05)
{
  ...
  close(pb) #pb
  write.xlsx(testmat, sprintf("data/%s -- %s.xlsx", 
